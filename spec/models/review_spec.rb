  ...
  describe 'valid?' do
    context 'presence' do
      it 'texto de avaliação em branco' do
        ...
        review = reservation.reviews.build(review_text: '', review_grade: 5, user_id: user.id)
        # Act
        review.valid?
      ...
      it 'nota da avaliação em branco' do
        ...
        special_prices = room.price_per_periods.create!(special_price: 10_000, starts_at: 1.day.ago, ends_at: 4.days.from_now)
        reservation = room.reservations.create!(user: user, room: room, check_in: Time.zone.now, check_out: 8.days.from_now, guests: 3)
        review = reservation.reviews.build(review_text: 'Algum texto', review_grade: '', user_id: user.id)
        # Act
        review.valid?
    ...
    context 'uniqueness' do   
      it 'usuário tenta fazer 2 reviews para a mesma reserva' do
        # Arrange
        user = User.create!(email: 'gmkoeb@gmail.com', password: 'password', name: 'Gabriel', 
        registration_number: '99999999999', admin: true)
        inn = Inn.create!(corporate_name: 'Pousadas Florianópolis LTDA', brand_name: 'Pousada do Luar', 
        ...
        special_prices = room.price_per_periods.create!(special_price: 10_000, starts_at: 1.day.ago, ends_at: 4.days.from_now)
        reservation = room.reservations.create!(user: user, room: room, check_in: Time.zone.now, check_out: 8.days.from_now, guests: 3)
        review_1 = reservation.reviews.create!(review_text: 'Texto de review', review_grade: 5, user_id: user.id)
        review_2 = reservation.reviews.build(review_text: 'Texto de review 2', review_grade: 5, user_id: user.id)
        # Act
        review_2.valid?
        result = review_2.errors.include?(:reservation_id)
        # Assert
        expect(result).to eq true
        expect(review_2.errors[:reservation_id]).to include 'essa reserva já foi avaliada'
      end
    end
  end
end
